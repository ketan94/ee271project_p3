...
//	
//
//  Source file: /home/ketan94/tars/assigns/ee271_tmp/assignment3/verif/rast_driver.vp
//  Source template: rast_driver
//
...
module rast_driver_unq1 
 (
   ...
   output logic signed [24-1:0] 	poly_R10S[3-1:0][3-1:0] , // Output: 4 Sets X,Y Fixed Point Values
   output logic                         isQuad_R10H,                     // Output: Is Poly Quad?
   output logic signed [24-1:0] 	color_R10U[3-1:0],
   output logic                         validPoly_R10H,                  // Output: Valid Data for Operation
   output logic signed [24-1:0]     screen_RnnnnS[1:0],              // Output: Screen Dimensions
   output logic        [3:0]            subSample_RnnnnU,                // Output: SubSample_Interval
   output int                       ss_w_lg2_RnnnnS   
);


   logic   signed   [24-1:0]  tmp_poly_R10S[3-1:0][3-1:0] ; // Output: 4 Sets X,Y Fixed Point Values
   logic   unsigned [24-1:0]  tmp_color_R10U[3-1:0];
   ...
   task InitLines;
	 ...
	 for( eachAxis = 0 ; eachAxis < 3 ; eachAxis++ ) begin
               for( eachVerts = 0 ; eachVerts < 3 ; eachVerts++) begin
		  poly_R10S[eachVerts][eachAxis] = $random();
	       end
	 end

	 for( eachColor = 0 ; eachColor < 3 ; eachColor++ ) begin
   ...
   task RunTest;
	 ...
	 while (!$feof(fh)) begin
	    ...
	    for( eachVertsA = 0 ; eachVertsA < 3 ; eachVertsA++ ) begin
